...
void setup(void)
{
  ...
  } else if ((serialMode == 3) || (serialMode == 5)) { // SBUS / MULTI
...
static uint16_t calculateMultiChannel(uint16_t input) {
  ...
  if (frameIndex == 0) {
    ...
    if (frameIndex == 25) {
      if ((ppmWork.sbus.status & 0x08) == 0) {
        uint8_t set;
        for (set = 0; set < 2; set++) {
          PPM[(set << 3)] = calculateMultiChannel(ppmWork.sbus.ch[set].ch0);
          PPM[(set << 3) + 1] = calculateMultiChannel(ppmWork.sbus.ch[set].ch1);
          PPM[(set << 3) + 2] = calculateMultiChannel(ppmWork.sbus.ch[set].ch2);
          PPM[(set << 3) + 3] = calculateMultiChannel(ppmWork.sbus.ch[set].ch3);
          PPM[(set << 3) + 4] = calculateMultiChannel(ppmWork.sbus.ch[set].ch4);
          PPM[(set << 3) + 5] = calculateMultiChannel(ppmWork.sbus.ch[set].ch5);
          PPM[(set << 3) + 6] = calculateMultiChannel(ppmWork.sbus.ch[set].ch6);
          PPM[(set << 3) + 7] = calculateMultiChannel(ppmWork.sbus.ch[set].ch7);
        }
